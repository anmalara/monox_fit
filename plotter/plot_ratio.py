...
def plot_ratio(process, category, model_file, outdir, lumi, year):
    ...
    for bgtype in bgtypes:
        ...
        for b in range(ratio.GetNbinsX() + 1):
            ...
            band_ewkqcdunc.SetBinError(b, sqrt(unc_dict["ewk"][b] ** 2 + unc_dict["qcd"][b] ** 2 + unc_dict["stat"][b] ** 2))
        ...
        latex2.DrawLatex(0.87, 0.95, "{LUMI:.1f} fb^{{-1}} (13.6 TeV)".format(LUMI=lumi))
