...
def convert_to_combine_workspace(
    ...
    for cat in categories:
        ...
        for crn in controlregions_def:
            ...
            try:
                for obj in cr_def.convertHistograms:
                    name = obj.GetName()
                    logger.info(f"Importing external histogram {name} from {crn} for category {cat}")
                    dhist = ROOT.RooDataHist(f"{cat}_{name}", f"DataSet - {cat}, {name}", ROOT.RooArgList(varl), obj)
                    wsin_combine._import(dhist)
            except:
                logger.critical(f"Failed to import control region definition: {crn}", exception_cls=ValueError)
                # continue
