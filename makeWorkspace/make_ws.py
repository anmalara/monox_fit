...
def cli_args():
    ...
    parser.add_argument(
        "--out", type=str, help="Path to save output under.", default="mono-x.root"
    )
    parser.add_argument(
        "--categories", type=str, default=None, help="Analysis category"
    )
    parser.add_argument(
        "--standalone",
        default=False,
        action="store_true",
        help="Treat this as a standalone conversion.",
    )
    parser.add_argument(
        "--indir", default=None, type=str, help="Input directory in the input file."
    )
...
def get_jes_file(category):
    ...
    f_jes_dict = {
        "(monoj|monov).*": ROOT.TFile(
            "sys/monoj_monov_shape_jes_uncs_smooth_{}.root".format(jer_suffix)
        ),
    ...
    if not f_jes:
        raise RuntimeError(
            "Could not find a JES source file for category: {}".format(category)
        )
...
def get_signal_theory_variations(obj, category):
    ...
    for unctype in "pdf", "scale":
        for direction in "Up", "Down":
            filler = {
                "CHANNEL": channel,
                "PROCESS_FOR_UNC": process_for_unc,
                "UNCTYPE": unctype,
                "DIRECTION": direction,
            ...
            if unctype == "scale":
                name = "signal_{REAL_PROCESS}_QCDscale_{REAL_PROCESS}_ACCEPT{DIRECTION}".format(
                    **filler
                )
            elif unctype == "pdf":
                name = (
                    "signal_{REAL_PROCESS}_pdf_{REAL_PROCESS}_ACCEPT{DIRECTION}".format(
                        **filler
                    )
                )

            varname = "{CHANNEL}_{PROCESS_FOR_UNC}_{UNCTYPE}{DIRECTION}".format(
                **filler
            )
...
def get_stat_variations(obj, category):
    ...
    for ibin in range(1, obj.GetNbinsX() + 1):
        variation_name_up = "{NAME}_{category}_stat_bin{ibin}Up".format(
            category=category, NAME=name, ibin=ibin
        )
        variation_name_dn = "{NAME}_{category}_stat_bin{ibin}Down".format(
            category=category, NAME=name, ibin=ibin
        )
...
def get_mergedMC_stat_variations(to_merge_mc_bkgs, category):
    ...
    for region_name in to_merge_mc_bkgs.keys():
        ...
        for ibin in range(1, merged_obj.GetNbinsX() + 1):
            variation_name_up = "{MERGED_NAME}_{category}_stat_bin{ibin}Up".format(
                category=category, MERGED_NAME=merged_name, ibin=ibin
            )
            variation_name_dn = "{MERGED_NAME}_{category}_stat_bin{ibin}Down".format(
                category=category, MERGED_NAME=merged_name, ibin=ibin
            )
            ...
            for obj in to_merge_mc_bkgs[region_name]:
                name = obj.GetName()
                name_up = "{NAME}_{VARIATION}".format(
                    NAME=name, VARIATION=variation_name_up
                )
                name_dn = "{NAME}_{VARIATION}".format(
                    NAME=name, VARIATION=variation_name_dn
                )
...
def mistag_processes(name):
    ...
    if (
        ...
        or name.startswith("Zmm")
        or name.startswith("Zee")
        or name.endswith("zll")
    ):
        processes.append("z")
    if "gjets" in name:
        processes.append("g")
    ...
    for sf_wp in "loose", "tight":
        ...
        for variation_index in range(2):
            ...
            for proc in mistag_processes(name):
                ...
                variation_name = (
                    "CMS_eff{YEAR}_vmistag_{PROC}_stat_{SF_WP}_{INDEX}".format(**filler)
                )
                for direction in "up", "down":
                    var = f.Get(
                        "{PROC}_{SF_WP}_{YEAR}_{CHANNEL}_var{INDEX}_{DIRECTION}".format(
                            DIRECTION=direction, **filler
                        )
                    )
                    var = scale_variation_histogram(var, scale)
                    varied_name = "{NAME}_{VARIATION}{DIRECTION}".format(
                        NAME=name,
...
def create_workspace(fin, fout, category, args):
    ...
    for key in keys_local:
        ...
        if not "data" in name:
            ...
            # TODO: check why no vbf in sys/photon_id_unc.root
            if "gjets" in key.GetName() and not "vbf" in category:
            ...
            # mistag variations
            # TODO: check why no vbf in sys/mistag_sf_variations.root
